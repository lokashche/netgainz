...
class CoachCommissionRun(Document):
	...
	def populate_lines(self):
		...
		self.post_to_ledger = 1 if frappe.db.get_single_value("Gym Settings", "commission_post_to_ledger") else 0
...
def create_commission_run(period_start=None, period_end=None) -> str:
	...
	doc = frappe.new_doc("Coach Commission Run")
	if period_start:
