...
def _horizon_days() -> int:
	raw = frappe.db.get_single_value("Business Settings", "class_schedule_horizon_days")
	try:
		horizon = int(raw)
...
def generate_scheduled_classes():
	...
	if not frappe.db.get_single_value("Business Settings", "class_auto_generate"):
		return None

	horizon = _horizon_days()
	total = 0
	for name in frappe.get_all("Session Schedule", filters={"is_active": 1}, pluck="name"):
		total += frappe.get_doc("Session Schedule", name).generate(horizon)
...
def generate_classes_now(name=None) -> dict:
	...
	if name:
		return {"created": frappe.get_doc("Session Schedule", name).generate()}

	total = 0
	for sched_name in frappe.get_all("Session Schedule", filters={"is_active": 1}, pluck="name"):
		total += frappe.get_doc("Session Schedule", sched_name).generate()
	return {"created": total}
