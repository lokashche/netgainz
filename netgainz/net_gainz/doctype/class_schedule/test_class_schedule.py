...
class TestClassSchedule(FrappeTestCase):
	...
	def tearDown(self):
		for sched in frappe.get_all(
			"Class Schedule", filters={"title": "Morning Batch"}, pluck="name"
		):
			for sess in frappe.get_all(
				"Class Session", filters={"class_schedule": sched}, pluck="name"
			):
				frappe.delete_doc("Class Session", sess, ignore_permissions=True, force=True)
			frappe.delete_doc("Class Schedule", sched, ignore_permissions=True, force=True)
	...
	def test_generate_is_idempotent(self):
		...
		first = frappe.db.count("Class Session", {"class_schedule": doc.name})
		self.assertGreaterEqual(first, 1)
		# Every generated session must be a Monday.
		for s in frappe.get_all(
			"Class Session", filters={"class_schedule": doc.name}, fields=["start_time"]
		):
			self.assertEqual(frappe.utils.getdate(s.start_time).weekday(), 0)

		again = doc.generate()
		self.assertEqual(again, 0)
		self.assertEqual(frappe.db.count("Class Session", {"class_schedule": doc.name}), first)
