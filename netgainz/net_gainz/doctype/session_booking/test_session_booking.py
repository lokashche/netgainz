...
class TestClassBooking(FrappeTestCase):
	...
	def tearDown(self):
		for name in frappe.get_all(
			"Session Booking", filters={"class_session": self.session.name}, pluck="name"
		):
			frappe.delete_doc("Session Booking", name, ignore_permissions=True, force=True)
		frappe.delete_doc("Session", self.session.name, ignore_permissions=True, force=True)
	...
	def test_check_in_stamped_on_attended(self):
		...
		doc = frappe.new_doc("Session Booking")
		doc.status = "Attended"
		doc.stamp_check_in()
