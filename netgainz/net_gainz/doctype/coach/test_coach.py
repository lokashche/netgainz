...
class TestCoach(FrappeTestCase):
	def test_status_default_is_active(self):
		...
		self.assertEqual(frappe.get_meta("Coach").get_field("status").default, "Active")

	def test_status_options(self):
		"""Coach status is limited to Active / Inactive."""
		options = frappe.get_meta("Coach").get_field("status").options
	...
	def test_commission_type_options(self):
		...
		options = frappe.get_meta("Coach").get_field("commission_type").options
