...
// Idempotently provisions the coach-commission ledger accounts. Safe to click
...
export default function SetupAccountsButton() {
  ...
  async function setup() {
    if (
      !confirm(
        "Create the \"Coach Commission Expense\" and \"Coach Commissions Payable\" accounts in your chart of accounts and map them? Safe to run more than once."
      )
    )
      return;
    setBusy(true);
    ...
    setMsg(null);
    try {
      const res = await fetch("/api/commissions/setup-accounts", { method: "POST" });
